...
waitForElement("body", (docBody) => {
    ...
    waitForElement("#block-pebl-content .banner-layout", () => {
        ...
        const interval = setInterval(() => {
            ...
            if (checkbox) {
                document
                    .querySelectorAll(".spz_3022_v1 .mktoFieldWrap textarea")
                    .forEach((textarea) => {
                ...
                if (typeof MktoForms2 !== 'undefined') {
                    MktoForms2.whenReady(function (form) {
                        ...
                        // Remove required attributes from input
                        phoneInput.removeAttr('required').removeAttr('aria-required');

                        // Remove mktoRequiredField class from wrapper
                        phoneFieldWrap.removeClass('mktoRequiredField');

                        // Remove form-required class from label
                        phoneLabel.removeClass('form-required');

                        // Remove asterisk if it exists
                        phoneLabel.find('.asterisk').remove();

                        // Override validation to allow empty phone
                        form.onValidate(function () {
                            const phoneElem = phoneInput.get(0);
                            // Always clear phone field errors
