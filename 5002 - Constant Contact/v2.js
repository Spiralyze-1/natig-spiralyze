...
hiddenValue('#5002 | Constant Contact | Buy Now | Additional Fields - Build', 'SPZ_5002_V2')
/***********************************
************************************
DO NOT TOUCH
BEYOND THIS LINE
...
function addForm() {
    ...
    document.body.classList.add('spz_5002_v2');
    ...
    if (!document.querySelector('.spz_dummy_cta')) {
        ...
        document.querySelector('.field__password-wrapper').parentElement.insertAdjacentHTML('afterend', `
            <div class="custom_field_wrapper">
                <div class="first_name_field input_wrapper">
                    <label class="label-spz" for="first_name">First Name (optional)</label>
        ...
        async function init() {
            ...
            // ADD THIS - Make country selector non-tabbable to fix tab order
            if (countrySelector) {
            ...
            // First Name validation (OPTIONAL in V2)
            ...
            if (firstNameInput) {
                const validateFirstName = () => {
                    ...
                    if (value.length === 0) {
                        errorMsg.innerHTML = "";
                        firstNameInput.classList.remove("invalid", "valid");
                        if (wrapper) wrapper.classList.remove('error');
                        return true;
                    }
                ...
                firstNameInput.addEventListener("keyup", validateFirstName); // ADD THIS LINE - validate while typing
            }
        }
        (async function run() {
            try {
                ...
                if (phoneInput) {
                    ...
                    const savedPhone = sessionStorage.getItem('spz_phone_value_v2');
                    if (savedPhone) {
                        ...
                        sessionStorage.removeItem('spz_phone_value_v2');
                    }

                    // Save to sessionStorage before leaving
                    window.addEventListener('beforeunload', () => {
                        if (phoneInput.value) {
                            sessionStorage.setItem('spz_phone_value_v2', phoneInput.value);
                        }
                    });
        ...
        // Add tab direction tracking for phone field focus fix
        ...
        document.querySelectorAll('.custom_field_wrapper input').forEach(input => {
            ...
            input.addEventListener('blur', () => {
                ...
                if (input.id === 'phone') {
                    ...
                    // For first name (optional) - let validation handle error state
                    if (input.value !== '') {
                        input.closest('.input_wrapper').classList.add('has-value');
        ...
        // Dummy cta
        document.querySelector('.spz_5002_v2 [type="submit"]').insertAdjacentHTML('afterend', `<div class="spz_dummy_cta" tabindex="0">Create my account</div>`);
        document.querySelector('.spz_dummy_cta').addEventListener('click', () => {
            let canSubmit = true;

            // Only validate if fields are visible (not hidden)
            if (!document.querySelector('.custom_field_wrapper').classList.contains('spz-hidden')) {
                const firstNameInput = document.querySelector('#first_name');
                const phoneInput = document.querySelector('#phone');

                // Validate First Name only if it has content
                ...
                // Validate Phone only if it has content
            ...
            if (canSubmit) {
                document.querySelector('.spz_5002_v2 [type="submit"]').click();
            }
