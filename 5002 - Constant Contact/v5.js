...
hiddenValue('#5002 | Constant Contact | Buy Now | Additional Fields - Build', 'SPZ_5002_V5')
/***********************************
************************************
DO NOT TOUCH
BEYOND THIS LINE
...
function addForm() {
    ...
    document.body.classList.add('spz_5002_v5');
    ...
    if (!document.querySelector('.spz_dummy_cta')) {
        ...
        async function init() {
            ...
            // Phone validation (REQUIRED in V5)
            const validate = () => {
                ...
                // If empty, show required error (required field in V5)
            ...
            // First Name validation (REQUIRED in V5)
        ...
        (async function run() {
            try {
                ...
                if (phoneInput) {
                    ...
                    const savedPhone = sessionStorage.getItem('spz_phone_value_v5');
                    if (savedPhone) {
                        ...
                        sessionStorage.removeItem('spz_phone_value_v5');
                    }

                    // Save to sessionStorage before leaving
                    window.addEventListener('beforeunload', () => {
                        if (phoneInput.value) {
                            sessionStorage.setItem('spz_phone_value_v5', phoneInput.value);
                        }
                    });
        ...
        document.querySelectorAll('.custom_field_wrapper input').forEach(input => {
            ...
            input.addEventListener('blur', () => {
                ...
                // Special handling for phone field (required)
                if (input.id === 'phone') {
                    if (input.value !== '') {
                        input.closest('.input_wrapper').classList.add('has-value');
        ...
        document.querySelector('.spz_5002_v5 [type="submit"]').insertAdjacentHTML('afterend', `
        ...
        document.querySelector('.spz_dummy_cta').addEventListener('click', () => {
            let canSubmit = true;
            const globalError = document.querySelector('.spz-global-error'); // ADD THIS

            const emailInput = document.querySelector('#email');
            const passwordInput = document.querySelector('#new-password');
            const emailFieldWrapper = document.querySelector('.field[data-attr-form-field-id="email"]');
            const passwordFieldWrapper = document.querySelector('.field__password-wrapper')?.parentElement;

            // === VALIDATE EMAIL ===
            if (emailInput && emailFieldWrapper) {
                const emailValue = emailInput.value.trim();
                const muiInputBase = emailFieldWrapper.querySelector('.MuiInputBase-root');
                ...
                if (!emailValue) {
                    ...
                    if (muiInputBase) muiInputBase.classList.add('Mui-error');
                    ...
                    if (muiInputBase) muiInputBase.classList.remove('Mui-error');
                    if (emailErrorContainer) {
            ...
            if (passwordInput && passwordFieldWrapper) {
                ...
                const muiInputBase = passwordFieldWrapper.querySelector('.MuiInputBase-root');
            ...
            // Only submit if all validation passed
            if (canSubmit) {
                ...
                document.querySelector('.spz_5002_v5 [type="submit"]').click();
            } else {
        ...
        const passwordInput = document.querySelector('#new-password');
        const emailFieldWrapper = document.querySelector('.field[data-attr-form-field-id="email"]');
        const passwordFieldWrapper = passwordInput ? document.querySelector('.field__password-wrapper')?.parentElement : null;
